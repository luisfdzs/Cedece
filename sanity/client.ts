 ...
 *
 * No es un `export const client = createClient(...)` porque aquí el proyecto de Sanity es
 * opcional: `createClient` con `projectId` vacío lanza al importar el módulo, y eso
 * reventaría el build de una web que no necesita Sanity para funcionar. Con una función,
 * el cliente sólo se construye cuando alguien va a consultar de verdad, y
 * `lib/content.ts` ya ha comprobado antes que hay con qué.
 *
