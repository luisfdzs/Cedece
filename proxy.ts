 ...
 * (`en-GB` → `en`, `es-AR` → `es`): a alguien que descubre a Cedecé desde Londres su
 * navegador le manda `en-GB`, que no coincidiría con un cotejo literal contra `en`.
 */
...
export const config = {
  ...
  // lo dejaría inaccesible. `video`, `gallery` y `posters` son ficheros de `public/`.
  matcher: [
    '/((?!api|admin|_next|video|gallery|posters|favicon|robots.txt|sitemap.xml|.*\\.[\\w]+$).*)',
  ],
}
